...
$(document).ready(function() {
    ...
    $.ajax({
      ...
      if (length == 0) {
        ...
        for (var i = 0; i < length; i++) {
          ...
          if (
            currentResult.name.toLowerCase() === park ||
            currentResult.fullname.toLowerCase() === park
          ) {
            console.log("working");
        ...
        var queryURL = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&units=imperial&appid=${key}`;
        ...
        // Forecast Call
        var forecastURL = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${long}&units=imperial&appid=${key}`;
        ...
        $.ajax({
          ...
          var row2 = $("#row2");
          var day1Date = $("<th>").text(
            moment.unix(day1.dt).format("MMM Do YYYY")
          );
          var day2Date = $("<th>").text(
            moment.unix(day2.dt).format("MMM Do YYYY")
          );
          var day4Date = $("<th>").text(
            moment.unix(day4.dt).format("MMM Do YYYY")
          );
          var day3Date = $("<th>").text(
            moment.unix(day3.dt).format("MMM Do YYYY")
          );
          var day5Date = $("<th>").text(
            moment.unix(day5.dt).format("MMM Do YYYY")
          );
          ...
          var row4 = $("#row4");
          var day1WindSpeed = $("<td>").text(
            `Wind Speed: ${Math.round(day1.wind.speed)} mph`
          );
          var day2WindSpeed = $("<td>").text(
            `Wind Speed: ${Math.round(day2.wind.speed)} mph`
          );
          var day3WindSpeed = $("<td>").text(
            `Wind Speed: ${Math.round(day3.wind.speed)} mph`
          );
          var day4WindSpeed = $("<td>").text(
            `Wind Speed: ${Math.round(day4.wind.speed)} mph`
          );
          var day5WindSpeed = $("<td>").text(
            `Wind Speed: ${Math.round(day5.wind.speed)} mph`
          );
        ...
        $.ajax({
          ...
          for (i = 0; i < results.length; i++) {
            var btn = $("<a>")
              .addClass("waves-effect waves-light btn hike orange accent-3")
              .text(results[i].name)
              .css("display", "block");
  ...
  function currentWeatherCall() {
    var queryURL = `https://api.openweathermap.org/data/2.5/weather?q=${city},usa&units=imperial&appid=${key}`;
  ...
  function forecastCall() {
    var queryURL = `https://api.openweathermap.org/data/2.5/forecast?q=${city},usa&units=imperial&appid=${key}`;
    console.log(queryURL);
    $.ajax({
      url: queryURL,
      method: "GET"
  ...
  $("#search-cities").on("click", function() {
    ...
    if (states.indexOf(state) === -1) {
      ...
      var currentResult = $(
        `<h1 class="center-align white-text">${searchResult}</h1>`
      );
      $("#current-result").show();
  ...
  database.ref("/Image").on("child_added", function(snap) {
    ...
    var title = $("<span>")
      ...
      .text(snap.val().title);
    var search = $("<i>")
      .addClass("material-icons black-text")
      .text("search");
    var btn = $("<a>")
      .addClass("btn-floating halfway-fab waves-effect waves-light fav-search")
      .css({
